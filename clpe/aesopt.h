/* aesopt.h
 * You can find the original source file in:
...
/*
 ...
 that is common across encryption, key scheduling and table generation.
    ...
    little endian internal byte order (see aesaes.h).  It inputs block and key
    ...
    clpe_aes_fret        (a signed 16 bit type for function return values)
    CLPE_AES_GOOD        (value != 0, a good return)
    CLPE_AES_BAD         (value == 0, an error return)
    struct aes_ctx  (structure for the cipher encryption context)
    struct aes_ctx  (structure for the cipher decryption context)
    clpe_aes_rval        the function return type (clpe_aes_fret if not DLL)

    C subroutine calls:

      clpe_aes_rval aes_blk_len(unsigned int blen, clpe_aes_ctx_t cx[1]);
      clpe_aes_rval aes_enc_key(const unsigned char in_key[], unsigned int klen, clpe_aes_ctx_t cx[1]);
      clpe_aes_rval aes_enc_blk(const unsigned char in_blk[], unsigned char out_blk[], const clpe_aes_ctx_t cx[1]);

      clpe_aes_rval aes_dec_len(unsigned int blen, clpe_aes_ctx_t cx[1]);
      clpe_aes_rval aes_dec_key(const unsigned char in_key[], unsigned int klen, clpe_aes_ctx_t cx[1]);
      clpe_aes_rval aes_dec_blk(const unsigned char in_blk[], unsigned char out_blk[], const clpe_aes_ctx_t cx[1]);
    ...
    C++ aes class subroutines:
      ...
      clpe_aes_rval len(unsigned int blen = 16);
      clpe_aes_rval key(const unsigned char in_key[], unsigned int klen);
      clpe_aes_rval blk(const unsigned char in_blk[], unsigned char out_blk[]);
    ...
    a. aesaes.h for the definitions needed for use in C.
    ...
    To compile AES (Rijndael) for use in C code use aesaes.h and exclude
    the CLPE_AES_DLL define in aesaes.h

    To compile AES (Rijndael) for use in in C++ code use aescpp.h and
    exclude the CLPE_AES_DLL define in aesaes.h

    To compile AES (Rijndael) in C as a Dynamic Link Library DLL) use
    aesaes.h, include the CLPE_AES_DLL define and compile the DLL.  If using
    ...
    CONFIGURATION OPTIONS (here and in aesaes.h)

    a. define CLPE_AES_BLOCK_SIZE in aesaes.h to set the cipher block size (16, 24
    ...
    b. set CLPE_AES_DLL in aesaes.h if AES (Rijndael) is to be compiled as a DLL
...
#  endif
#endif
#endif /* !defined(WORDS_BIGENDIAN) && !defined(WORDS_LITTLEENDIAN) */
...
#if defined(CLPE_AES_BLOCK_SIZE) && ((CLPE_AES_BLOCK_SIZE & 3) || CLPE_AES_BLOCK_SIZE < 16 || CLPE_AES_BLOCK_SIZE > 32)
...
#define RC_LENGTH    29
#else
#define RC_LENGTH   5 * CLPE_AES_BLOCK_SIZE / 4 - (CLPE_AES_BLOCK_SIZE == 16 ? 10 : 11)
#endif
...
#if defined(_MSC_VER)
#define upr(x,n)        _lrotl((uint32_t)(x), 8 * (n))
#else
...
#if !defined(bswap_32)
#if !defined(_MSC_VER)
#define _lrotl(x,n)     ((uint32_t)(x) <<  n | (uint32_t)(x) >> 32 - n)
#endif
...
prefx uint32_t  aes_rcon_tab[29];
...
#ifdef  FT1_SET
prefx uint32_t aes_ft_tab[256];
#endif

#ifdef  FT4_SET
prefx uint32_t aes_ft_tab[4][256];
#endif

#ifdef  FL1_SET
prefx uint32_t aes_fl_tab[256];
#endif

#ifdef  FL4_SET
prefx uint32_t aes_fl_tab[4][256];
#endif

#ifdef  IT1_SET
prefx uint32_t aes_it_tab[256];
#endif

#ifdef  IT4_SET
prefx uint32_t aes_it_tab[4][256];
#endif

#ifdef  IL1_SET
prefx uint32_t aes_il_tab[256];
#endif

#ifdef  IL4_SET
prefx uint32_t aes_il_tab[4][256];
#endif
...
#ifdef  IM1_SET
prefx uint32_t aes_im_tab[256];
#endif

#ifdef  IM4_SET
prefx uint32_t aes_im_tab[4][256];
#endif
...
#if defined(CLPE_AES_BLOCK_SIZE)
#define nc  (CLPE_AES_BLOCK_SIZE >> 2)
#else
...
#define dec_imvars
#if defined(IM4_SET)
#define inv_mcol(x)     four_tables(x,aes_im_tab,vf1,rf1,0)
#elif defined(IM1_SET)
#define inv_mcol(x)     one_table(x,upr,aes_im_tab,vf1,rf1,0)
#else
#undef  dec_imvars
...
#if defined(FL4_SET)
#define ls_box(x,c)     four_tables(x,aes_fl_tab,vf1,rf2,c)
#elif   defined(LS4_SET)
#define ls_box(x,c)     four_tables(x,ls_tab,vf1,rf2,c)
#elif defined(FL1_SET)
#define ls_box(x,c)     one_table(x,upr,aes_fl_tab,vf1,rf2,c)
