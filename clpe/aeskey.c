...
#if defined(CLPE_AES_BLOCK_SIZE) && (CLPE_AES_BLOCK_SIZE & 7)
...
clpe_aes_rval clpe_aes_blk_len(unsigned int blen, clpe_aes_ctx_t cx[1])
{
    ...
    {
        cx->n_blk = 0; return CLPE_AES_BAD;
    }

    cx->n_blk = blen;
    return CLPE_AES_GOOD;
}
...
#define ke4(k,i) \
{   k[4*(i)+4] = ss[0] ^= ls_box(ss[3],3) ^ aes_rcon_tab[i]; k[4*(i)+5] = ss[1] ^= ss[0]; \
...
#define ke6(k,i) \
{   k[6*(i)+ 6] = ss[0] ^= ls_box(ss[5],3) ^ aes_rcon_tab[i]; k[6*(i)+ 7] = ss[1] ^= ss[0]; \
...
#define ke8(k,i) \
{   k[8*(i)+ 8] = ss[0] ^= ls_box(ss[7],3) ^ aes_rcon_tab[i]; k[8*(i)+ 9] = ss[1] ^= ss[0]; \
...
clpe_aes_rval
clpe_aes_enc_key(const unsigned char in_key[], unsigned int klen, clpe_aes_ctx_t cx[1])
{   uint32_t    ss[8];
...
#if (CLPE_AES_BLOCK_SIZE == 16) && (ENC_UNROLL != NONE)
    ...
    case 32:    cx->k_sch[4] = ss[4] = word_in(in_key + 16);
                ...
                kel8(cx->k_sch, 6);
                cx->n_rnd = 14; break;
    default:    cx->n_rnd = 0; return CLPE_AES_BAD;
    }
#else
    {   uint32_t i, l;
...
{   ss[0] = ss[0] ^ ss[2] ^ ss[1] ^ ss[3]; ss[1] = ss[1] ^ ss[3]; ss[2] = ss[2] ^ ss[3]; ss[3] = ss[3]; \
    ss[4] = ls_box(ss[(i+3) % 4], 3) ^ aes_rcon_tab[i]; ss[i % 4] ^= ss[4]; \
    ss[4] ^= k[4*(i)];   k[4*(i)+4] = ff(ss[4]); ss[4] ^= k[4*(i)+1]; k[4*(i)+5] = ff(ss[4]); \
    ss[4] ^= k[4*(i)+2]; k[4*(i)+6] = ff(ss[4]); ss[4] ^= k[4*(i)+3]; k[4*(i)+7] = ff(ss[4]); \
}
#define kd4(k,i) \
{   ss[4] = ls_box(ss[(i+3) % 4], 3) ^ aes_rcon_tab[i]; ss[i % 4] ^= ss[4]; ss[4] = ff(ss[4]); \
...
#else
#define kdf4(k,i) \
{   ss[0] ^= ls_box(ss[3],3) ^ aes_rcon_tab[i]; k[4*(i)+ 4] = ff(ss[0]); ss[1] ^= ss[0]; k[4*(i)+ 5] = ff(ss[1]); \
    ss[2] ^= ss[1]; k[4*(i)+ 6] = ff(ss[2]); ss[3] ^= ss[2]; k[4*(i)+ 7] = ff(ss[3]); \
}
#define kd4(k,i) \
{   ss[4] = ls_box(ss[3],3) ^ aes_rcon_tab[i]; \
...
#define kdl4(k,i) \
{   ss[0] ^= ls_box(ss[3],3) ^ aes_rcon_tab[i]; k[4*(i)+ 4] = ss[0]; ss[1] ^= ss[0]; k[4*(i)+ 5] = ss[1]; \
    ss[2] ^= ss[1]; k[4*(i)+ 6] = ss[2]; ss[3] ^= ss[2]; k[4*(i)+ 7] = ss[3]; \
}
#endif

#define kdf6(k,i) \
{   ss[0] ^= ls_box(ss[5],3) ^ aes_rcon_tab[i]; k[6*(i)+ 6] = ff(ss[0]); ss[1] ^= ss[0]; k[6*(i)+ 7] = ff(ss[1]); \
    ss[2] ^= ss[1]; k[6*(i)+ 8] = ff(ss[2]); ss[3] ^= ss[2]; k[6*(i)+ 9] = ff(ss[3]); \
    ss[4] ^= ss[3]; k[6*(i)+10] = ff(ss[4]); ss[5] ^= ss[4]; k[6*(i)+11] = ff(ss[5]); \
}
#define kd6(k,i) \
{   ss[6] = ls_box(ss[5],3) ^ aes_rcon_tab[i]; \
...
#define kdl6(k,i) \
{   ss[0] ^= ls_box(ss[5],3) ^ aes_rcon_tab[i]; k[6*(i)+ 6] = ss[0]; ss[1] ^= ss[0]; k[6*(i)+ 7] = ss[1]; \
    ss[2] ^= ss[1]; k[6*(i)+ 8] = ss[2]; ss[3] ^= ss[2]; k[6*(i)+ 9] = ss[3]; \
}

#define kdf8(k,i) \
{   ss[0] ^= ls_box(ss[7],3) ^ aes_rcon_tab[i]; k[8*(i)+ 8] = ff(ss[0]); ss[1] ^= ss[0]; k[8*(i)+ 9] = ff(ss[1]); \
    ss[2] ^= ss[1]; k[8*(i)+10] = ff(ss[2]); ss[3] ^= ss[2]; k[8*(i)+11] = ff(ss[3]); \
    ss[4] ^= ls_box(ss[3],0); k[8*(i)+12] = ff(ss[4]); ss[5] ^= ss[4]; k[8*(i)+13] = ff(ss[5]); \
    ss[6] ^= ss[5]; k[8*(i)+14] = ff(ss[6]); ss[7] ^= ss[6]; k[8*(i)+15] = ff(ss[7]); \
}
#define kd8(k,i) \
{   uint32_t g = ls_box(ss[7],3) ^ aes_rcon_tab[i]; \
...
#define kdl8(k,i) \
{   ss[0] ^= ls_box(ss[7],3) ^ aes_rcon_tab[i]; k[8*(i)+ 8] = ss[0]; ss[1] ^= ss[0]; k[8*(i)+ 9] = ss[1]; \
    ss[2] ^= ss[1]; k[8*(i)+10] = ss[2]; ss[3] ^= ss[2]; k[8*(i)+11] = ss[3]; \
}

clpe_aes_rval
clpe_aes_dec_key(const unsigned char in_key[], unsigned int klen, clpe_aes_ctx_t cx[1])
{   uint32_t    ss[8];
    d_vars
...
#if (CLPE_AES_BLOCK_SIZE == 16) && (DEC_UNROLL != NONE)
